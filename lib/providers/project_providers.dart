...
final database_service_provider = Provider<DatabaseService>((ref) { // Provides a value but its not reactive on its own, it's mainly for dependency injection of the DatabaseService instance.
...
final projects_provider = StreamProvider<List<Project>>((ref) { // Use StreamProvider because we want to listen to a stream of data (the list of projects for the current user) that can change over time, and we want the UI to automatically update whenever there are changes to the projects in the Firestore database.
  final DatabaseService databaseService = ref.watch(database_service_provider); // ref.watch: "get the database service instance from the provider, so we can call methods on it to get the project data stream.""
  return databaseService.streamProjectsForCurrentUser(); // calls the streamProjectsForCurrentUser method of the DatabaseService to get a stream of lists of Project instances that belong to the currently authenticated user. This stream will emit new values whenever there are changes to the projects in the Firestore database, allowing the UI to reactively update with the latest project data.
});
...
final selected_project_provider = StateProvider<Project?>((ref) { // Use StateProvider because we want to store a single piece of state (the currently selected project) that can be updated and accessed from different parts of the app. The UI can read this state to know which project is currently selected, and it can also update this state when the user selects a different project.
