...
class _AddLaborEntryDialogState extends ConsumerState<AddLaborEntryDialog> {
  ...
  Future<void> _submit() async {
    ...
    final double? hours = double.tryParse( // double.tryParse attempts to convert the input text as a double, and returns null if the input is not a valid number. This allows us to validate that the user has entered a valid numeric value for hours before trying to create the labor entry.
      _hoursController.text.trim(), // We use .trim() to remove any leading or trailing whitespace from the input before trying to parse it as a number, which helps prevent parsing errors if the user accidentally includes extra spaces.
              ...
              children: <Widget>[
                TextFormField(
                  ...
                  validator: (String? value) {
                    if (value == null || value.trim().isEmpty) { // Validate that the role/task field is not empty. If it is empty, return an error message that will be displayed below the input field. If it is valid, return null to indicate no validation error.
